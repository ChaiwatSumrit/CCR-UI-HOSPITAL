...
@Injectable()
export class CcrDocumentService {
  ...
  submitCreateCcrDocument(model: TransactionCreateCcrDocument): Observable<any> {
    const url = environment.backendBaseUrl + '/ccr/public/document/create';
  ...
  submitSaveCcrDocument(model: TransactionCreateCcrDocument): Observable<any> {
    const url = environment.backendBaseUrl + '/ccr/public/document/save';
    let headers = new Headers();
    this.createAuthorizationHeader(headers);
    return this.http.post(url, model, {
      headers: headers
