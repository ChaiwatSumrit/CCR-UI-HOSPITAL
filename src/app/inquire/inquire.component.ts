...
//import buildPDF from 'assets/ildPDF/buildPDF';
...
export class InquireComponent implements OnInit {
  ...
  dowloadPdf() {
    ...
    pdfMake.createPdf(this.docDefinition).download(`CcrDocument`);
  }
      ...
      .subscribe(result => {
        ...
        for (var rows = 0; rows < result.message.receipt.treatment.length; rows++) {
          let body = {
            ...
            "Price": (result.message.receipt.treatment[rows].price).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,'),
            "Amount": (result.message.receipt.treatment[rows].amount).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')
          }
